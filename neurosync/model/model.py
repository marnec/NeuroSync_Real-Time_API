...
def apply_rope(x, seq_len):
    ...
    dim_indices = torch.arange(0, d_model, 2, dtype=torch.float, device=x.device)  # Shape: (d_model // 2)
    div_term = torch.exp(-torch.log(torch.tensor(10000.0, device=x.device)) * dim_indices / d_model)  # Shape: (d_model // 2)
    
    # Compute sine and cosine components
    angle = position * div_term  # Shape: (seq_len, d_model // 2)
    sin = torch.sin(angle).unsqueeze(0)  # Shape: (1, seq_len, d_model // 2)
    cos = torch.cos(angle).unsqueeze(0)  # Shape: (1, seq_len, d_model // 2)

    # Split input tensor into even and odd dimensions
    x1, x2 = x[..., ::2], x[..., 1::2]  # Shapes: (batch_size, seq_len, d_model // 2)

    # Apply rotation
    x_rope_even = x1 * cos - x2 * sin
    x_rope_odd = x1 * sin + x2 * cos

    # Interleave the results (recombine even and odd indices)
...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        x = apply_rope(x, seq_len)  # Apply Rotary Position Embedding
        
        outputs = self.transformer_encoder(x)  # Pass through transformer encoder
...
class Decoder(nn.Module):
    ...
    def forward(self, encoder_outputs):
        seq_len = encoder_outputs.size(1)  # Sequence length
        input = apply_rope(encoder_outputs, seq_len)  # Apply Rotary Position Embedding
        
        decoder_output = self.transformer_decoder(input, encoder_outputs)  # Transformer decoder

        if self.use_norm:
            decoder_output = self.layer_norm(decoder_output)
        
        prediction = self.fc_output(decoder_output)  # Final prediction
        return prediction
'''
'''
